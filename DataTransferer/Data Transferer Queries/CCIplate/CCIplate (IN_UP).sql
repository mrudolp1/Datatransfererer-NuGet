--Model Declarations (confirmed)
DECLARE @Model TABLE(ModelID INT)
DECLARE @ModelID INT
--Structure Info Declarations (confirmed)
DECLARE @BU VARCHAR(10)
DECLARE @STR_ID VARCHAR(10)
DECLARE @ModelNeeded BIT --NEW
--DECLARE @STR_TYPE VARCHAR(50)
--Connection Group Declarations (NEW) --CCIplate
DECLARE @Congrp TABLE(CongrpID INT)
DECLARE @CongrpID INT
--Plate Connections Declarations --CCIplate
DECLARE @Connection TABLE(CID INT)
DECLARE @CID INT
--Plate Details Declarations --CCIplate
DECLARE @PlateDetail TABLE(PDID INT)
DECLARE @PDID INT
...
DECLARE @ConGroupNeeded BIT --NEW CCIPLATE
...
DECLARE @PileNeeded BIT --NEW
	...
	Set @ConGroupNeeded = '[Con GRP ID Needed]'
...
IF @ConGroupNeeded = 1 --TRUE (Reference isfndGroupNeeded)
BEGIN
	...
	INSERT INTO conn.connection_group OUTPUT Inserted.ID INTO @Congrp DEFAULT VALUES
	SELECT @CongrpID=CongrpID FROM @Congrp
	UPDATE gen.structure_model Set connection_group_id=@CongrpID WHERE ID=@ModelID
...
--NEGLECT EVERYTHING BELOW


--	--INSERT Plate Connections
--		INSERT INTO conn.plate_connections VALUES ([INSERT ALL PLATE CONNECTIONS])
--		--INSERT INTO conn.plate_connections (connection_group_id,connection_elevation,connection_type,bolt_configuration) OUTPUT INSERTED.ID INTO @Connection VALUES ([INSERT ALL PLATE CONNECTIONS])
--		SELECT @CID=CID FROM @Connection
--		INSERT INTO conn.plate_connetions_xref (connection_group_id,connection_id) VALUES (@CongrpID,@CID)
--	End
--End

...
#Region "Define"
    ...
    Private Property CCIpoleTemplatePath As String = "C:\Users\" & Environment.UserName & "\Desktop\C Drive Testing\CCIpole\EDS Testing\CCIpole (4.6.0.1) - TEMPLATE.xlsm"
...
#Region "Load Data"
    Sub CreateSQLPoles(ByRef poleList As List(Of CCIpole))
        ...
        If IsSomething(ds.Tables("CCIpole General SQL")) Then
            For Each CCIpoleDataRow As DataRow In ds.Tables("CCIpole General SQL").Rows 'MRR - Need help...No Details Table, main object is just the pole_structure_id and criteria_id
...
#Region "Save Data"
    ...
    Sub Save1Pole(ByVal cp As CCIpole)
        ...
        For Each cppc As PoleCriteria In cp.criteria
            ...
            If cppc.criteria_id = 0 Or IsDBNull(cppc.criteria_id) Then
                CCIpoleSaver = CCIpoleSaver.Replace("'[Pole Criteria ID]'", "NULL")
                CCIpoleSaver = CCIpoleSaver.Replace("--CriteriaDNU ", "")

                CCIpoleSaver = CCIpoleSaver.Replace("[upper_structure_type]", cppc.upper_structure_type.ToString)
                CCIpoleSaver = CCIpoleSaver.Replace("[analysis_deg]", cppc.analysis_deg.ToString)
                CCIpoleSaver = CCIpoleSaver.Replace("[geom_increment_length]", cppc.geom_increment_length.ToString)
                CCIpoleSaver = CCIpoleSaver.Replace("[vnum]", cppc.vnum.ToString)
                CCIpoleSaver = CCIpoleSaver.Replace("[check_connections]", cppc.check_connections.ToString)
                CCIpoleSaver = CCIpoleSaver.Replace("[hole_deformation]", cppc.hole_deformation.ToString)
                CCIpoleSaver = CCIpoleSaver.Replace("[ineff_mod_check]", cppc.ineff_mod_check.ToString)
                CCIpoleSaver = CCIpoleSaver.Replace("[modified]", cppc.modified.ToString)
            Else
                CCIpoleSaver = CCIpoleSaver.Replace("'[Pole Criteria ID]'", cppc.criteria_id)
            End If
        ...
        For Each cpps As PoleSection In cp.unreinf_sections
            ...
            SubQuery_BGeom = SubQuery_BGeom.TrimEnd()

            myPoleSection = tempPoleSection

            SubQuery_BGeom = SubQuery_BGeom.Replace("'[INSERT SINGLE POLE SECTION]'", myPoleSection)

            'MATL DB
            If cpps.steel_grade_id = 0 Or IsDBNull(cpps.steel_grade_id) Then
                SubQuery_BGeom = SubQuery_BGeom.Replace("'[STEEL GRADE ID]'", "NULL")
                SubQuery_BGeom = SubQuery_BGeom.Replace("--MatlDNU ", "")

                For Each cpm As PropMatl In cp.matls
                    If cpm.local_id = cpps.local_matl_id Then 'And cpps.local_matl_id > 17 Then 'Matching, Non-Standard Materials
                        SubQuery_BGeom = SubQuery_BGeom.Replace("[local_id]", cpm.local_id.ToString)
                        SubQuery_BGeom = SubQuery_BGeom.Replace("[name]", cpm.name.ToString)
                        SubQuery_BGeom = SubQuery_BGeom.Replace("[fy]", cpm.fy.ToString)
                        SubQuery_BGeom = SubQuery_BGeom.Replace("[fu]", cpm.fu.ToString)
            ...
            Else
                SubQuery_BGeom = SubQuery_BGeom.Replace("[STEEL GRADE ID]", cpps.steel_grade_id.ToString)
                'SubQuery_BGeom = SubQuery_BGeom.Replace("IF @MatlID IS NULL" & vbNewLine & "BEGIN" & vbNewLine & "IF EXISTS(SELECT * FROM pole.matl_prop_flat_plate WHERE local_id = '[local_id]' AND name = '[name]' AND fy = '[fy]' AND fu = '[fu]')" & vbNewLine & "SELECT @MatlID = ID FROM pole.matl_prop_flat_plate WHERE local_id = '[local_id]' AND name = '[name]' AND fy = '[fy]' AND fu = '[fu]'" & vbNewLine & "ELSE" & vbNewLine & "BEGIN" & vbNewLine & "INSERT INTO pole.matl_prop_flat_plate OUTPUT INSERTED.ID INTO @PropMatl VALUES ('[INSERT MATL PROP]')" & vbNewLine & "SELECT @MatlID = MatlID FROM @PropMatl" & vbNewLine & "END" & vbNewLine & "END", "--IF @MatlID IS NULL")
        ...
        For Each cprs As PoleReinfSection In cp.reinf_sections
            ...
            Dim SubQuery_RGeom As String = QueryBuilderFromFile(queryPath & "CCIpole\CCIpole (SubQuery_GeomReinf).sql")
            SubQuery_RGeom = SubQuery_RGeom.TrimEnd()

            myPoleReinfSection = tempPoleReinfSection

            SubQuery_RGeom = SubQuery_RGeom.Replace("'[INSERT SINGLE POLE SECTION]'", myPoleReinfSection)

            'MATL DB
            If cprs.steel_grade_id = 0 Or IsDBNull(cprs.steel_grade_id) Then
                SubQuery_RGeom = SubQuery_RGeom.Replace("'[STEEL GRADE ID]'", "NULL")
                SubQuery_RGeom = SubQuery_RGeom.Replace("--MatlDNU ", "")

                For Each cpm As PropMatl In cp.matls
                    If cpm.local_id = cprs.local_matl_id Then 'And cprs.local_matl_id > 17 Then 'Matching, Non-Standard Materials
                        SubQuery_RGeom = SubQuery_RGeom.Replace("[local_id]", cpm.local_id.ToString)
                        SubQuery_RGeom = SubQuery_RGeom.Replace("[name]", cpm.name.ToString)
                        SubQuery_RGeom = SubQuery_RGeom.Replace("[fy]", cpm.fy.ToString)
                        SubQuery_RGeom = SubQuery_RGeom.Replace("[fu]", cpm.fu.ToString)

                        Dim tempPropMatl As String = InsertPropMatl(cpm)
                        myMatlProp = tempPropMatl

                        SubQuery_RGeom = SubQuery_RGeom.Replace("'[INSERT MATL PROP]'", myMatlProp)

                    End If
                Next

            Else
                SubQuery_RGeom = SubQuery_RGeom.Replace("[STEEL GRADE ID]", cprs.steel_grade_id.ToString)
                'SubQuery_RGeom = SubQuery_RGeom.Replace("IF @MatlID IS NULL" & vbNewLine & "BEGIN" & vbNewLine & "IF EXISTS(SELECT * FROM pole.matl_prop_flat_plate WHERE local_id = '[local_id]' AND name = '[name]' AND fy = '[fy]' AND fu = '[fu]')" & vbNewLine & "SELECT @MatlID = ID FROM pole.matl_prop_flat_plate WHERE local_id = '[local_id]' AND name = '[name]' AND fy = '[fy]' AND fu = '[fu]'" & vbNewLine & "ELSE" & vbNewLine & "BEGIN" & vbNewLine & "INSERT INTO pole.matl_prop_flat_plate OUTPUT INSERTED.ID INTO @PropMatl VALUES ('[INSERT MATL PROP]')" & vbNewLine & "SELECT @MatlID = MatlID FROM @PropMatl" & vbNewLine & "END" & vbNewLine & "END", "--IF @MatlID IS NULL")
        ...
        For Each cprg As PoleReinfGroup In cp.reinf_groups
            ...
            Dim subQuery_RGrp As String = QueryBuilderFromFile(queryPath & "CCIpole\CCIpole (SubQuery_ReinfGroups).sql")
            subQuery_RGrp = subQuery_RGrp.TrimEnd()

            myReinfGroup = tempPoleReinfGroup

            subQuery_RGrp = subQuery_RGrp.Replace("'[INSERT SINGLE REINF GROUP]'", myReinfGroup)

            'REINFORCEMENT DB
            If cprg.reinf_db_id = 0 Or IsDBNull(cprg.reinf_db_id) Then
                subQuery_RGrp = subQuery_RGrp.Replace("'[REINFORCEMENT ID]'", "NULL")
                subQuery_RGrp = subQuery_RGrp.Replace("--ReinfDNU ", "")

                For Each cpr As PropReinf In cp.reinfs
                    If cpr.local_id = cprg.local_reinf_id Then 'And cprg.local_reinf_id > 58 Then 'Matching, Non-Standard Materials
                        Dim tempPropReinf As String = InsertPropReinf(cpr)

                        myReinfProp = tempPropReinf

                        subQuery_RGrp = subQuery_RGrp.Replace("'[INSERT REINF PROP]'", myReinfProp)

                        'MATL DB
                        If cpr.matl_id = 0 Or IsDBNull(cpr.matl_id) Then
                            subQuery_RGrp = subQuery_RGrp.Replace("'[STEEL GRADE ID]'", "NULL")
                            subQuery_RGrp = subQuery_RGrp.Replace("--MatlDNU ", "")

                            For Each cpm As PropMatl In cp.matls
                                If cpm.local_id = cpr.local_matl_id Then 'And cpr.local_matl_id > 17 Then 'Matching, Non-Standard Materials
                                    subQuery_RGrp = subQuery_RGrp.Replace("[local_id]", cpm.local_id.ToString)
                                    subQuery_RGrp = subQuery_RGrp.Replace("[name]", cpm.name.ToString)
                                    subQuery_RGrp = subQuery_RGrp.Replace("[fy]", cpm.fy.ToString)
                                    subQuery_RGrp = subQuery_RGrp.Replace("[fu]", cpm.fu.ToString)

                                    Dim tempPropMatl As String = InsertPropMatl(cpm)
                                    myMatlProp = tempPropMatl

                                    subQuery_RGrp = subQuery_RGrp.Replace("'[INSERT MATL PROP]'", myMatlProp)
                                End If
                            Next
                            myMatlProp = ""
                        Else
                            subQuery_RGrp = subQuery_RGrp.Replace("[STEEL GRADE ID]", cpr.matl_id.ToString)
                            'subQuery_RGrp = subQuery_RGrp.Replace("IF @MatlID IS NULL" & vbNewLine & "BEGIN" & vbNewLine & "IF EXISTS(SELECT * FROM pole.matl_prop_flat_plate WHERE local_id = '[local_id]' AND name = '[name]' AND fy = '[fy]' AND fu = '[fu]')" & vbNewLine & "SELECT @MatlID = ID FROM pole.matl_prop_flat_plate WHERE local_id = '[local_id]' AND name = '[name]' AND fy = '[fy]' AND fu = '[fu]'" & vbNewLine & "ELSE" & vbNewLine & "BEGIN" & vbNewLine & "INSERT INTO pole.matl_prop_flat_plate OUTPUT INSERTED.ID INTO @PropMatl VALUES ('[INSERT MATL PROP]')" & vbNewLine & "SELECT @MatlID = MatlID FROM @PropMatl" & vbNewLine & "END" & vbNewLine & "END", "--IF @MatlID IS NULL")

                        End If

                        'BOLT - Bottom
                        If cpr.bolt_type_id_bot = 0 Or IsDBNull(cpr.bolt_type_id_bot) Then
                            Dim subsubQuery_Bolt As String = QueryBuilderFromFile(queryPath & "CCIpole\CCIpole (SubSubQuery_Bolts).sql")
                            subsubQuery_Bolt = subsubQuery_Bolt.TrimEnd()

                            subsubQuery_Bolt = subsubQuery_Bolt.Replace("@BoltID", "@BotBoltID")
                            subsubQuery_Bolt = subsubQuery_Bolt.Replace("'[BOLT ID]'", "NULL")

                            For Each cpb As PropBolt In cp.bolts
                                If cpb.local_id = cpr.local_bolt_id_bot Then 'And cpr.local_matl_id > 17 Then 'Matching, Non-Standard Materials
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[local_id]", cpb.local_id.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[name]", cpb.name.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[description]", cpb.description.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[diam]", cpb.diam.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[area]", cpb.area.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[fu_bolt]", cpb.fu_bolt.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[sleeve_diam_out]", cpb.sleeve_diam_out.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[sleeve_diam_in]", cpb.sleeve_diam_in.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[fu_sleeve]", cpb.fu_sleeve.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[bolt_n_sleeve_shear_revF]", cpb.bolt_n_sleeve_shear_revF.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[bolt_x_sleeve_shear_revF]", cpb.bolt_x_sleeve_shear_revF.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[bolt_n_sleeve_shear_revG]", cpb.bolt_n_sleeve_shear_revG.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[bolt_x_sleeve_shear_revG]", cpb.bolt_x_sleeve_shear_revG.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[bolt_n_sleeve_shear_revH]", cpb.bolt_n_sleeve_shear_revH.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[bolt_x_sleeve_shear_revH]", cpb.bolt_x_sleeve_shear_revH.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[rb_applied_revH]", cpb.rb_applied_revH.ToString)

                                    Dim tempPropBolt As String = InsertPropBolt(cpb)
                                    myBoltProp = tempPropBolt

                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("'[INSERT BOLT PROP]'", myBoltProp)
                                    subQuery_RGrp = subQuery_RGrp.Replace("--'[BOLT SUB-SUBQUERY]'", subsubQuery_Bolt)
                                End If
                            Next
                            myBoltProp = ""
                        Else
                            subQuery_RGrp = subQuery_RGrp.Replace("--'[BOLT SUB-SUBQUERY]'", "SET @BotBoltID = '" & cpr.bolt_type_id_bot & "'" & vbNewLine & "--'[BOLT SUB-SUBQUERY]'")
                        End If

                        'BOLT - Top
                        If cpr.bolt_type_id_top = 0 Or IsDBNull(cpr.bolt_type_id_top) Then
                            Dim subsubQuery_Bolt As String = QueryBuilderFromFile(queryPath & "CCIpole\CCIpole (SubSubQuery_Bolts).sql")
                            subsubQuery_Bolt = subsubQuery_Bolt.TrimEnd()

                            subsubQuery_Bolt = subsubQuery_Bolt.Replace("@BoltID", "@TopBoltID")
                            subsubQuery_Bolt = subsubQuery_Bolt.Replace("'[BOLT ID]'", "NULL")

                            For Each cpb As PropBolt In cp.bolts
                                If cpb.local_id = cpr.local_bolt_id_top Then 'And cpr.local_matl_id > 17 Then 'Matching, Non-Standard Materials
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[local_id]", cpb.local_id.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[name]", cpb.name.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[description]", cpb.description.ToString)
                                    subsubQuery_Bolt = subsubQuery_Bolt.Replace("[diam]", cpb.diam.ToString)
                            ...
                            myBoltProp = ""
                        Else
                            subQuery_RGrp = subQuery_RGrp.Replace("--'[BOLT SUB-SUBQUERY]'", "SET @TopBoltID = '" & cpr.bolt_type_id_bot & "'")
                        End If
            ...
            Else
                subQuery_RGrp = subQuery_RGrp.Replace("[REINFORCEMENT ID]", cprg.reinf_db_id.ToString) 'Standard Reinfs populate EDS ID automatically in tool - Standard Reinf plates use standard Bolts and Materials

                If cprg.reinf_db_id <= 10 Then
                    subQuery_RGrp = subQuery_RGrp.Replace("[STEEL GRADE ID]", 6)
                Else
                    subQuery_RGrp = subQuery_RGrp.Replace("[STEEL GRADE ID]", 12)
                End If

                'subQuery_RGrp = subQuery_RGrp.Replace("IF @ReinfID IS NULL" & vbNewLine & "BEGIN" & vbNewLine & "INSERT INTO pole.memb_prop_flat_plate OUTPUT INSERTED.ID INTO @PropReinf VALUES ('[INSERT REINF PROP]')" & vbNewLine & "SELECT @ReinfID = ReinfID FROM @PropReinf" & vbNewLine & "END", "--IF @ReinfID IS NULL")
                'subQuery_RGrp = subQuery_RGrp.Replace("IF @MatlID IS NULL" & vbNewLine & "BEGIN" & vbNewLine & "IF EXISTS(SELECT * FROM pole.matl_prop_flat_plate WHERE local_id = '[local_id]' AND name = '[name]' AND fy = '[fy]' AND fu = '[fu]')" & vbNewLine & "SELECT @MatlID = ID FROM pole.matl_prop_flat_plate WHERE local_id = '[local_id]' AND name = '[name]' AND fy = '[fy]' AND fu = '[fu]'" & vbNewLine & "ELSE" & vbNewLine & "BEGIN" & vbNewLine & "INSERT INTO pole.matl_prop_flat_plate OUTPUT INSERTED.ID INTO @PropMatl VALUES ('[INSERT MATL PROP]')" & vbNewLine & "SELECT @MatlID = MatlID FROM @PropMatl" & vbNewLine & "END" & vbNewLine & "END", "--IF @MatlID IS NULL")
                If cprg.reinf_db_id >= 25 And cprg.reinf_db_id <= 38 Then
                    subQuery_RGrp = subQuery_RGrp.Replace("--'[BOLT SUB-SUBQUERY]'", "SET @BotBoltID = NULL" & vbNewLine & "SET @TopBoltID = 1")
                Else
                    subQuery_RGrp = subQuery_RGrp.Replace("--'[BOLT SUB-SUBQUERY]'", "SET @BotBoltID = 1" & vbNewLine & "SET @TopBoltID = 1")
                End If
            ...
            For Each cprd As PoleReinfDetail In cprg.reinf_ids
                Dim subsubQuery_ReinfDetail As String = QueryBuilderFromFile(queryPath & "CCIpole\CCIpole (SubSubQuery_ReinfDetails).sql")
                subsubQuery_ReinfDetail = subsubQuery_ReinfDetail.TrimEnd()

                Dim tempPoleReinfDetail As String = InsertPoleReinfDetail(cprd)

                myReinfDetail = tempPoleReinfDetail

                subsubQuery_ReinfDetail = subsubQuery_ReinfDetail.Replace("'[INSERT REINF DETAILS]'", myReinfDetail)
                subQuery_RGrp = subQuery_RGrp.Replace("--'[DETAIL SUB-SUBQUERY]'", subsubQuery_ReinfDetail)
            Next
        ...
        For Each cpig As PoleIntGroup In cp.int_groups
            ...
            Dim subQuery_IGrp As String = QueryBuilderFromFile(queryPath & "CCIpole\CCIpole (SubQuery_IntGroups).sql")
            subQuery_IGrp = subQuery_IGrp.TrimEnd()

            myIntGroup = tempPoleIntGroup

            subQuery_IGrp = subQuery_IGrp.Replace("'[INSERT SINGLE INT GROUP]'", myIntGroup)

            'INTERFERENCE DETAIL
            For Each cpid As PoleIntDetail In cpig.int_ids
                Dim subsubQuery_IntDetail As String = QueryBuilderFromFile(queryPath & "CCIpole\CCIpole (SubSubQuery_IntDetails).sql")
                subsubQuery_IntDetail = subsubQuery_IntDetail.TrimEnd()

                Dim tempPoleIntDetail As String = InsertPoleIntDetail(cpid)

                myIntDetail = tempPoleIntDetail

                subsubQuery_IntDetail = subsubQuery_IntDetail.Replace("'[INSERT INTERFERENCE DETAILS]'", myIntDetail)
                subQuery_IGrp = subQuery_IGrp.Replace("--'[DETAIL SUB-SUBQUERY]'", subsubQuery_IntDetail)
            Next

            CCIpoleSaver = CCIpoleSaver.Replace("--'[SUBQUERY]'", subQuery_IGrp)
    ...
    Public Sub SaveToExcel()
        ...
        For Each cp As CCIpole In Poles
            ...
            With NewCCIpoleWb
                ...
                For Each prg As PoleReinfGroup In cp.reinf_groups
                    ...
                    For Each prd As PoleReinfDetail In prg.reinf_ids
                        ...
                        If prd.local_group_id = prg.local_group_id Then
                            ...
                            dcol += 1
                            If Not IsNothing(prd.local_reinf_id) Then .Worksheets("Reinf ID (SAPI)").Cells(drow, dcol).Value = CType(prd.local_reinf_id, Integer)
                            dcol += 1
                ...
                For Each pig As PoleIntGroup In cp.int_groups
                    ...
                    For Each pid As PoleIntDetail In pig.int_ids
                        ...
                        If pid.local_group_id = pig.local_group_id Then
                            ...
                            dcol += 1
                            If Not IsNothing(pid.int_id) Then .Worksheets("Interference ID (SAPI)").Cells(drow, dcol).Value = CType(pid.int_id, Integer)
                            dcol += 1
                            ...
                            dcol += 1
                            If Not IsNothing(pid.local_int_id) Then .Worksheets("Interference ID (SAPI)").Cells(drow, dcol).Value = CType(pid.local_int_id, Integer)
                            dcol += 1
                ...
                For Each prr As PoleReinfResults In cp.reinf_section_results
                    ...
                    col += 1
                    If Not IsNothing(prr.local_reinf_group_id) Then .Worksheets("Reinf Results (SAPI)").Cells(row, col).Value = CType(prr.local_reinf_group_id, Integer)
                    col += 1
                ...
                For Each pr As PropReinf In cp.reinfs
                    ...
                    If pr.reinf_db_id > 58 Then
                        ...
                        col += 1
                        If Not IsNothing(pr.local_id) Then .Worksheets("Reinforcements (SAPI)").Cells(row, col).Value = CType(pr.local_id, Integer)
                        col += 1
                        ...
                        col += 1
                        '
                        If Not IsNothing(pr.bolt_type_id_bot) Then .Worksheets("Reinforcements (SAPI)").Cells(row, col).Value = CType(pr.bolt_type_id_bot, Integer)
                        col += 1
                        ...
                        col += 1
                        '
                        If Not IsNothing(pr.bolt_type_id_top) Then .Worksheets("Reinforcements (SAPI)").Cells(row, col).Value = CType(pr.bolt_type_id_top, Integer)
                        col += 1
                ...
                For Each pb As PropBolt In cp.bolts
                    ...
                    If pb.bolt_db_id > 11 Then

                        col = 0

                        If Not IsNothing(cp.pole_structure_id) Then .Worksheets("Bolts (SAPI)").Cells(row, col).Value = CType(cp.pole_structure_id, Integer)
                        col += 1
                        If Not IsNothing(pb.bolt_db_id) Then .Worksheets("Bolts (SAPI)").Cells(row, col).Value = CType(pb.bolt_db_id, Integer)
                        col += 1
                        If Not IsNothing(pb.local_id) Then .Worksheets("Bolts (SAPI)").Cells(row, col).Value = CType(pb.local_id, Integer)
                        col += 1
                        ...
                        col += 1
                        If Not IsNothing(pb.rb_applied_revH) Then .Worksheets("Bolts (SAPI)").Cells(row, col).Value = pb.rb_applied_revH
                ...
                For Each pm As PropMatl In cp.matls
                    ...
                    If pm.matl_db_id > 17 Then

                        col = 0

                        If Not IsNothing(cp.pole_structure_id) Then .Worksheets("Materials (SAPI)").Cells(row, col).Value = CType(cp.pole_structure_id, Integer)
                        col += 1
                        If Not IsNothing(pm.matl_db_id) Then .Worksheets("Materials (SAPI)").Cells(row, col).Value = CType(pm.matl_db_id, Integer)
                        col += 1
                        If Not IsNothing(pm.local_id) Then .Worksheets("Materials (SAPI)").Cells(row, col).Value = CType(pm.local_id, Integer)
                        col += 1
                ...
                .Worksheets("Macro References").Range("ID_pole").Value = CType(cp.pole_structure_id, Integer)
        ...
        'Call EDS_Connection_Import 'MRR
        '
...
#Region "SQL Insert Statements"
    ...
    Private Function InsertPoleSection(ByVal ps As PoleSection) As String
        ...
        insertString += "," & IIf(IsNothing(ps.local_matl_id), "Null", ps.local_matl_id.ToString)
    ...
    Private Function InsertPoleReinfSection(ByVal prs As PoleReinfSection) As String
        ...
        insertString += ",@MatlID" 'IIf(IsNothing(prs.steel_grade_id), "Null", prs.steel_grade_id.ToString)
        insertString += "," & IIf(IsNothing(prs.local_matl_id), "Null", prs.local_matl_id.ToString)
    ...
    Private Function InsertPoleReinfGroup(ByVal prg As PoleReinfGroup) As String
        ...
        'insertString += "@PoleID"
        insertString += IIf(IsNothing(prg.local_group_id), "Null", prg.local_group_id.ToString)
        ...
        insertString += ",@ReinfID" 'IIf(IsNothing(prg.reinf_db_id), "Null", prg.reinf_db_id.ToString)  IF 18 or Less then set to NULL in sub sub query
        insertString += "," & IIf(IsNothing(prg.local_reinf_id), "Null", prg.local_reinf_id.ToString)
        insertString += "," & IIf(IsNothing(prg.qty), "Null", prg.qty.ToString)
    ...
    Private Function InsertPoleReinfDetail(ByVal prd As PoleReinfDetail) As String
        ...
        'insertString += "@ReinfGroupID"
        insertString += IIf(IsNothing(prd.local_group_id), "Null", prd.local_group_id.ToString)
        insertString += "," & IIf(IsNothing(prd.local_reinf_id), "Null", prd.local_reinf_id.ToString)
    ...
    Private Function InsertPoleIntGroup(ByVal pig As PoleIntGroup) As String
        ...
        'insertString += "@PoleID"
        insertString += IIf(IsNothing(pig.local_group_id), "Null", pig.local_group_id.ToString)
        ...
        insertString += "," & IIf(IsNothing(pig.qty), "Null", pig.qty.ToString)
    ...
    Private Function InsertPoleIntDetail(ByVal pid As PoleIntDetail) As String
        ...
        'insertString += "@IntGroupID"
        insertString += IIf(IsNothing(pid.local_group_id), "Null", pid.local_group_id.ToString)
        insertString += "," & IIf(IsNothing(pid.local_int_id), "Null", pid.local_int_id.ToString)
    ...
    Private Function InsertPoleReinfResults(ByVal prr As PoleReinfResults) As String
        ...
        insertString += ",@ReinfGroupID"
        insertString += "," & IIf(IsNothing(prr.local_section_id), "Null", prr.local_section_id.ToString)
        insertString += "," & IIf(IsNothing(prr.local_reinf_group_id), "Null", prr.local_reinf_group_id.ToString)
    ...
    Private Function InsertPropReinf(ByVal pr As PropReinf) As String
        ...
        insertString += IIf(IsNothing(pr.local_id), "Null", pr.local_id.ToString)
        ...
        insertString += ",@MatlID" '& IIf(IsNothing(pr.matl_id), "Null", pr.matl_id.ToString)
        insertString += "," & IIf(IsNothing(pr.local_matl_id), "Null", pr.local_matl_id.ToString)
        ...
        insertString += ",@BotBoltID" '& IIf(IsNothing(pr.bolt_type_id_bot), "Null", pr.bolt_type_id_bot.ToString)
        insertString += "," & IIf(IsNothing(pr.local_bolt_id_bot), "Null", pr.local_bolt_id_bot.ToString)
        ...
        insertString += ",@TopBoltID" '& IIf(IsNothing(pr.bolt_type_id_top), "Null", pr.bolt_type_id_top.ToString)
        insertString += "," & IIf(IsNothing(pr.local_bolt_id_top), "Null", pr.local_bolt_id_top.ToString)
        ...
        insertString += "," & IIf(IsNothing(pr.conn_length_channel), "Null", pr.conn_length_channel.ToString)
    ...
    Private Function InsertPropBolt(ByVal pb As PropBolt) As String
        ...
        'insertString += "@BoltID"
        insertString += IIf(IsNothing(pb.local_id), "Null", pb.local_id.ToString)
    ...
    Private Function InsertPropMatl(ByVal pm As PropMatl) As String
        ...
        'insertString += "@MatlID"
        insertString += IIf(IsNothing(pm.local_id), "Null", pm.local_id.ToString)
...
#Region "SQL Update Statements"
    ...
    Private Function UpdatePoleSection(ByVal ps As PoleSection) As String
        ...
        updateString += ", local_matl_id=" & IIf(IsNothing(ps.local_matl_id), "Null", ps.local_matl_id.ToString)
    ...
    Private Function UpdatePoleReinfSection(ByVal prs As PoleReinfSection) As String
        ...
        updateString += ", local_matl_id=" & IIf(IsNothing(prs.local_matl_id), "Null", prs.local_matl_id.ToString)
    ...
    Private Function UpdatePoleReinfGroup(ByVal prg As PoleReinfGroup) As String
        ...
        updateString += "local_group_id=" & IIf(IsNothing(prg.local_group_id), "Null", prg.local_group_id.ToString)
        ...
        updateString += ", local_reinf_id=" & IIf(IsNothing(prg.local_reinf_id), "Null", prg.local_reinf_id.ToString)
        updateString += ", qty=" & IIf(IsNothing(prg.qty), "Null", prg.qty.ToString)
        updateString += " WHERE ID = " & prg.group_id.ToString
    ...
    Private Function UpdatePoleReinfDetail(ByVal prd As PoleReinfDetail) As String
        ...
        updateString += "local_group_id=" & IIf(IsNothing(prd.local_group_id), "Null", prd.local_group_id.ToString)
        updateString += ", local_reinf_id=" & IIf(IsNothing(prd.local_reinf_id), "Null", prd.local_reinf_id.ToString)
    ...
    Private Function UpdatePoleIntGroup(ByVal pig As PoleIntGroup) As String
        ...
        updateString += "local_group_id=" & IIf(IsNothing(pig.local_group_id), "Null", pig.local_group_id.ToString)
        ...
        updateString += ", qty=" & IIf(IsNothing(pig.qty), "Null", pig.qty.ToString)
        updateString += " WHERE ID = " & pig.group_id.ToString
    ...
    Private Function UpdatePoleIntDetail(ByVal pid As PoleIntDetail) As String
        ...
        updateString += "local_group_id=" & IIf(IsNothing(pid.local_group_id), "Null", pid.local_group_id.ToString)
        updateString += ", local_int_id=" & IIf(IsNothing(pid.local_int_id), "Null", pid.local_int_id.ToString)
        ...
        updateString += " WHERE ID = " & pid.int_id.ToString
    ...
    Private Function UpdatePoleReinfResults(ByVal prr As PoleReinfResults) As String
        ...
        updateString += ", local_section_id=" & IIf(IsNothing(prr.local_section_id), "Null", prr.local_section_id.ToString)
        updateString += ", local_reinf_group_id=" & IIf(IsNothing(prr.local_reinf_group_id), "Null", prr.local_reinf_group_id.ToString)
    ...
    Private Function UpdatePropReinf(ByVal pr As PropReinf) As String
        ...
        updateString += "local_id=" & IIf(IsNothing(pr.local_id), "Null", pr.local_id.ToString)
        ...
        updateString += ", local_matl_id=" & IIf(IsNothing(pr.local_matl_id), "Null", pr.local_matl_id.ToString)
        ...
        updateString += ", local_bolt_id_bot=" & IIf(IsNothing(pr.local_bolt_id_bot), "Null", pr.local_bolt_id_bot.ToString)
        ...
        updateString += ", local_bolt_id_top=" & IIf(IsNothing(pr.local_bolt_id_top), "Null", pr.local_bolt_id_top.ToString)
        ...
        updateString += ", conn_length_channel=" & IIf(IsNothing(pr.conn_length_channel), "Null", pr.conn_length_channel.ToString)
    ...
    Private Function UpdatePropBolt(ByVal pb As PropBolt) As String
        ...
        updateString += "local_id=" & IIf(IsNothing(pb.local_id), "Null", pb.local_id.ToString)
    ...
    Private Function UpdatePropMatl(ByVal pm As PropMatl) As String
        ...
        updateString += "local_id=" & IIf(IsNothing(pm.local_id), "Null", pm.local_id.ToString)
...
#Region "General"
    ...
    Private Function CCIpoleExcelDTParameters() As List(Of EXCELDTParameter)
        ...
        MyParameters.Add(New EXCELDTParameter("CCIpole Criteria EXCEL", "A2:J3", "Analysis Criteria (SAPI)"))
        MyParameters.Add(New EXCELDTParameter("CCIpole Pole Sections EXCEL", "A2:W20", "Unreinf Pole (SAPI)"))
        MyParameters.Add(New EXCELDTParameter("CCIpole Pole Reinf Sections EXCEL", "A2:W200", "Reinf Pole (SAPI)"))
        MyParameters.Add(New EXCELDTParameter("CCIpole Reinf Groups EXCEL", "A2:J50", "Reinf Groups (SAPI)"))
        MyParameters.Add(New EXCELDTParameter("CCIpole Reinf Details EXCEL", "A2:H200", "Reinf ID (SAPI)"))
        MyParameters.Add(New EXCELDTParameter("CCIpole Int Groups EXCEL", "A2:H50", "Interference Groups (SAPI)"))
        MyParameters.Add(New EXCELDTParameter("CCIpole Int Details EXCEL", "A2:H200", "Interference ID (SAPI)"))
        MyParameters.Add(New EXCELDTParameter("CCIpole Pole Reinf Results EXCEL", "A2:G1000", "Reinf Results (SAPI)"))
        MyParameters.Add(New EXCELDTParameter("CCIpole Reinf Property Details EXCEL", "A2:EA50", "Reinforcements (SAPI)"))
    ...
    Function CheckChanges(ByVal xlPole As CCIpole, ByVal sqlPole As CCIpole) As Boolean
        ...
        For Each xlps As PoleSection In xlPole.unreinf_sections
            For Each sqlps As PoleSection In sqlPole.unreinf_sections
                If xlps.section_id = sqlps.section_id Then
                    ...
                    If Check1Change(xlps.local_matl_id, sqlps.local_matl_id, "CCIpole", "Local_Steel_Grade_Id " & xlps.section_id.ToString) Then changesMade = True
                ...
                ElseIf xlps.section_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them. 
                    ...
                    If Check1Change(xlps.local_matl_id, Nothing, "CCIpole", "Local_Steel_Grade_Id " & xlps.section_id.ToString) Then changesMade = True
        ...
        For Each xlprs As PoleReinfSection In xlPole.reinf_sections
            For Each sqlprs As PoleReinfSection In sqlPole.reinf_sections
                If xlprs.section_id = sqlprs.section_id Then
                    ...
                    If Check1Change(xlprs.local_matl_id, sqlprs.local_matl_id, "CCIpole", "Local_Steel_Grade_Id " & xlprs.section_id.ToString) Then changesMade = True
                ...
                ElseIf xlprs.section_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                    ...
                    If Check1Change(xlprs.local_matl_id, Nothing, "CCIpole", "Local_Steel_Grade_Id " & xlprs.section_id.ToString) Then changesMade = True
        ...
        For Each xlprg As PoleReinfGroup In xlPole.reinf_groups
            For Each sqlprg As PoleReinfGroup In sqlPole.reinf_groups
                If xlprg.group_id = sqlprg.group_id Then
                    If Check1Change(xlprg.local_group_id, sqlprg.local_group_id, "CCIpole", "local_group_id " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.elev_bot_actual, sqlprg.elev_bot_actual, "CCIpole", "Elev_Bot_Actual " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.elev_bot_eff, sqlprg.elev_bot_eff, "CCIpole", "Elev_Bot_Eff " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.elev_top_actual, sqlprg.elev_top_actual, "CCIpole", "Elev_Top_Actual " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.elev_top_eff, sqlprg.elev_top_eff, "CCIpole", "Elev_Top_Eff " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.reinf_db_id, sqlprg.reinf_db_id, "CCIpole", "Reinf_Db_Id " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.local_reinf_id, sqlprg.local_reinf_id, "CCIpole", "local_reinf_id " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.qty, sqlprg.qty, "CCIpole", "qty " & xlprg.group_id.ToString) Then changesMade = True
                    Exit For
                ElseIf xlprg.group_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                    If Check1Change(xlprg.local_group_id, Nothing, "CCIpole", "local_group_id " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.elev_bot_actual, Nothing, "CCIpole", "Elev_Bot_Actual " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.elev_bot_eff, Nothing, "CCIpole", "Elev_Bot_Eff " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.elev_top_actual, Nothing, "CCIpole", "Elev_Top_Actual " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.elev_top_eff, Nothing, "CCIpole", "Elev_Top_Eff " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.reinf_db_id, Nothing, "CCIpole", "Reinf_Db_Id " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.local_reinf_id, Nothing, "CCIpole", "local_reinf_id " & xlprg.group_id.ToString) Then changesMade = True
                    If Check1Change(xlprg.qty, Nothing, "CCIpole", "qty " & xlprg.group_id.ToString) Then changesMade = True
                    Exit For
                End If
                ...
                For Each xlprd As PoleReinfDetail In xlprg.reinf_ids
                    For Each sqlprd As PoleReinfDetail In sqlprg.reinf_ids
                        If xlprd.reinf_id = sqlprd.reinf_id Then
                            If Check1Change(xlprd.local_group_id, sqlprd.local_group_id, "CCIpole", "local_group_id " & xlprd.reinf_id.ToString) Then changesMade = True
                            If Check1Change(xlprd.local_reinf_id, sqlprd.local_reinf_id, "CCIpole", "local_reinf_id " & xlprd.reinf_id.ToString) Then changesMade = True
                        ...
                        ElseIf xlprd.reinf_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                            If Check1Change(xlprd.local_group_id, Nothing, "CCIpole", "local_group_id " & xlprd.reinf_id.ToString) Then changesMade = True
                            If Check1Change(xlprd.local_reinf_id, Nothing, "CCIpole", "local_reinf_id " & xlprd.reinf_id.ToString) Then changesMade = True
        ...
        For Each xlpig As PoleIntGroup In xlPole.int_groups
            For Each sqlpig As PoleIntGroup In sqlPole.int_groups
                If xlpig.group_id = sqlpig.group_id Then
                    If Check1Change(xlpig.local_group_id, sqlpig.local_group_id, "CCIpole", "local_group_id " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.elev_bot, sqlpig.elev_bot, "CCIpole", "Elev_Bot " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.elev_top, sqlpig.elev_top, "CCIpole", "Elev_Top " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.width, sqlpig.width, "CCIpole", "Width " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.description, sqlpig.description, "CCIpole", "Description " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.qty, sqlpig.qty, "CCIpole", "qty " & xlpig.group_id.ToString) Then changesMade = True
                    Exit For
                ElseIf xlpig.group_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                    If Check1Change(xlpig.local_group_id, Nothing, "CCIpole", "local_group_id " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.elev_bot, Nothing, "CCIpole", "Elev_Bot " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.elev_top, Nothing, "CCIpole", "Elev_Top " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.width, Nothing, "CCIpole", "Width " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.description, Nothing, "CCIpole", "Description " & xlpig.group_id.ToString) Then changesMade = True
                    If Check1Change(xlpig.qty, Nothing, "CCIpole", "qty " & xlpig.group_id.ToString) Then changesMade = True
                    Exit For
                End If
                ...
                For Each xlpid As PoleIntDetail In xlpig.int_ids
                    For Each sqlpid As PoleIntDetail In sqlpig.int_ids
                        If xlpid.int_id = sqlpid.int_id Then
                            If Check1Change(xlpid.local_group_id, sqlpid.local_group_id, "CCIpole", "local_group_id " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.local_int_id, sqlpid.local_int_id, "CCIpole", "local_int_id " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.pole_flat, sqlpid.pole_flat, "CCIpole", "Pole_Flat " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.horizontal_offset, sqlpid.horizontal_offset, "CCIpole", "Horizontal_Offset " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.rotation, sqlpid.rotation, "CCIpole", "Rotation " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.note, sqlpid.note, "CCIpole", "Note " & xlpid.int_id.ToString) Then changesMade = True
                            Exit For
                        ElseIf xlpid.int_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                            If Check1Change(xlpid.local_group_id, Nothing, "CCIpole", "local_group_id " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.local_int_id, Nothing, "CCIpole", "local_int_id " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.pole_flat, Nothing, "CCIpole", "Pole_Flat " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.horizontal_offset, Nothing, "CCIpole", "Horizontal_Offset " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.rotation, Nothing, "CCIpole", "Rotation " & xlpid.int_id.ToString) Then changesMade = True
                            If Check1Change(xlpid.note, Nothing, "CCIpole", "Note " & xlpid.int_id.ToString) Then changesMade = True
                            Exit For
                        End If
                    Next
                Next
        ...
        For Each xlprr As PoleReinfResults In xlPole.reinf_section_results
            For Each sqlprr As PoleReinfResults In sqlPole.reinf_section_results
                If xlprr.section_id = sqlprr.section_id Then
                    ...
                    If Check1Change(xlprr.local_section_id, sqlprr.local_section_id, "CCIpole", "local_section_id " & xlprr.section_id.ToString) Then changesMade = True
                    If Check1Change(xlprr.local_reinf_group_id, sqlprr.local_reinf_group_id, "CCIpole", "Local_Reinf_Group_Id " & xlprr.section_id.ToString) Then changesMade = True
                ...
                ElseIf xlprr.section_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                    ...
                    If Check1Change(xlprr.local_section_id, Nothing, "CCIpole", "local_section_id " & xlprr.section_id.ToString) Then changesMade = True
                    If Check1Change(xlprr.local_reinf_group_id, Nothing, "CCIpole", "Local_Reinf_Group_Id " & xlprr.section_id.ToString) Then changesMade = True
        ...
        For Each xlpr As PropReinf In xlPole.reinfs
            For Each sqlpr As PropReinf In sqlPole.reinfs
                If xlpr.reinf_db_id = sqlpr.reinf_db_id Then
                    If Check1Change(xlpr.local_id, sqlpr.local_id, "CCIpole", "local_id " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    ...
                    If Check1Change(xlpr.local_matl_id, sqlpr.local_matl_id, "CCIpole", "Local_Matl_Id " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    ...
                    If Check1Change(xlpr.bolt_type_id_bot, sqlpr.bolt_type_id_bot, "CCIpole", "bolt_id_Bot " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    If Check1Change(xlpr.local_bolt_id_bot, sqlpr.local_bolt_id_bot, "CCIpole", "local_bolt_id_bot " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    ...
                    If Check1Change(xlpr.bolt_type_id_top, sqlpr.bolt_type_id_top, "CCIpole", "bolt_id_Top " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    If Check1Change(xlpr.local_bolt_id_top, sqlpr.local_bolt_id_top, "CCIpole", "local_bolt_id_top " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    ...
                    If Check1Change(xlpr.conn_length_channel, sqlpr.conn_length_channel, "CCIpole", "conn_length_channel " & xlpr.reinf_db_id.ToString) Then changesMade = True
                ...
                ElseIf xlpr.reinf_db_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                    If Check1Change(xlpr.local_id, Nothing, "CCIpole", "Local_ID " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    ...
                    If Check1Change(xlpr.local_matl_id, Nothing, "CCIpole", "Local_Matl_Id " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    ...
                    If Check1Change(xlpr.bolt_type_id_bot, Nothing, "CCIpole", "bolt_id_Bot " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    If Check1Change(xlpr.local_bolt_id_bot, Nothing, "CCIpole", "local_bolt_id_bot " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    ...
                    If Check1Change(xlpr.bolt_type_id_top, Nothing, "CCIpole", "bolt_id_Top " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    If Check1Change(xlpr.local_bolt_id_top, Nothing, "CCIpole", "local_bolt_id_top " & xlpr.reinf_db_id.ToString) Then changesMade = True
                    ...
                    If Check1Change(xlpr.conn_length_channel, Nothing, "CCIpole", "conn_length_channel " & xlpr.reinf_db_id.ToString) Then changesMade = True
        ...
        For Each xlpb As PropBolt In xlPole.bolts
            For Each sqlpb As PropBolt In sqlPole.bolts
                If xlpb.bolt_db_id = sqlpb.bolt_db_id Then
                    If Check1Change(xlpb.local_id, sqlpb.local_id, "CCIpole", "local_id " & xlpb.bolt_db_id.ToString) Then changesMade = True
                ...
                ElseIf xlpb.bolt_db_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                    If Check1Change(xlpb.local_id, Nothing, "CCIpole", "local_id " & xlpb.bolt_db_id.ToString) Then changesMade = True
        ...
        For Each xlpm As PropMatl In xlPole.matls
            For Each sqlpm As PropMatl In sqlPole.matls
                If xlpm.matl_db_id = sqlpm.matl_db_id Then
                    If Check1Change(xlpm.local_id, sqlpm.local_id, "CCIpole", "local_id " & xlpm.matl_db_id.ToString) Then changesMade = True
                ...
                ElseIf xlpm.matl_db_id = 0 Then 'accounts for inserting new rows. additional rows won't have an ID associated to them.
                    If Check1Change(xlpm.local_id, Nothing, "CCIpole", "local_id " & xlpm.matl_db_id.ToString) Then changesMade = True

...
Partial Public Class DataTransfererDrilledPierIEM
...
#Region "Save Data"
    ...
    'Public Function CompareAgainstEDS() As Boolean
    '    If dp1.pier_depth <> dp2.pier_depth Then ChangesMade : changeLIst.Add("Pier Depth")
    'End Function

    'Function checkifequal(ByVal val1, ByVal val2, ByVal tolerance)
    '    If val1 + tolerance Then
    'End Function

#Region "Save to Internal Database"
    ...
    Public Sub SaveToExcel_DrilledPierInternalDatabase()
        Dim dpRow As Integer = 3
        Dim secCount As Integer = 1
        Dim rebCount As Integer = 1
        Dim soilCount As Integer = 1
        Dim embedSecCount As Integer = 1
        ...
        With NewDrilledPierWb
            For Each dp As DrilledPier In DrilledPiers
                colCounter = 6 + dp.local_pier_id
                myCol = GetExcelColumnName(colCounter)

                .Worksheets("Database").Range(myCol & 53).Value = dp.pier_id
                .Worksheets("Database").Range(myCol & 65).Value = dp.foundation_depth
                .Worksheets("Database").Range(myCol & 64).Value = dp.extension_above_grade
                .Worksheets("Database").Range(myCol & 70).Value = dp.groundwater_depth
                .Worksheets("Database").Range(myCol & 4442).Value = dp.assume_min_steel
                .Worksheets("Database").Range(myCol & 150).Value = dp.check_shear_along_depth
                .Worksheets("Database").Range(myCol & 153).Value = dp.embedded_pole
                .Worksheets("Database").Range(myCol & 165).Value = dp.belled_pier
                .Worksheets("Database").Range(myCol & 71).Value = dp.soil_layer_quantity
                '.Worksheets("Details (SAPI)").Range("H" & dpRow).Value = dp.utilize_skin_friction_methodology

                Dim depth As Integer = 65
                Dim secBump As Integer = 0
                Dim secStart As Integer = 73
                ''''''''''''DRILLED PIER SECTION
                For Each dpSec As DrilledPierSection In dp.sections
                    '.Worksheets("Sections (SAPI)").Range(myCol & secRow).Value = dp.pier_id
                    '.Worksheets("Sections (SAPI)").Range(myCol & secRow).Value = dpSec.section_id 'NEED TO FIND A WAY TO INCLUDE THIS
                    .Worksheets("Database").Range(myCol & secStart + 0).Value = dpSec.pier_diameter
                    .Worksheets("Database").Range(myCol & secStart + 3).Value = dpSec.clear_cover
                    .Worksheets("Database").Range(myCol & secStart + 14).Value = dpSec.clear_cover_rebar_cage_option
                    .Worksheets("Database").Range(myCol & secStart + 4).Value = dpSec.tie_size
                    .Worksheets("Database").Range(myCol & secStart + 5).Value = dpSec.tie_spacing

                    If secCount > 1 Then depth += 1
                    .Worksheets("Database").Range(myCol & depth).Value = dpSec.bottom_elevation
                    '.Worksheets("Sections (SAPI)").Range("J" & secRow).Value = dpSec.top_elevation
                    'If secCount = 1 Then
                    '    .Worksheets("Sections (SAPI)").Range("L" & 40).Value = dpSec.tie_yield_strength
                    '    .Worksheets("Sections (SAPI)").Range("M" & 60).Value = dpSec.concrete_compressive_strength
                    'End If
                    '.Worksheets("Sections (SAPI)").Range("N" & secRow).Value = dpSec.assum_min_steel_rho_override

                    ''''''''''''DRILLED PIER REBAR
                    For Each dpReb As DrilledPierRebar In dpSec.rebar
                        '.Worksheets("Rebar (SAPI)").Range(myCol & rebRow).Value = dp.pier_id
                        '.Worksheets("Rebar (SAPI)").Range(myCol & rebRow).Value = dpSec.section_id
                        '.Worksheets("Rebar (SAPI)").Range(myCol & rebRow).Value = dpReb.rebar_id 'NEED TO FIND A WAY TO INCLUDE THIS
                        If rebCount = 1 Then
                            .Worksheets("Database").Range(myCol & secStart + 1).Value = dpReb.longitudinal_rebar_quantity
                            .Worksheets("Database").Range(myCol & secStart + 2).Value = dpReb.longitudinal_rebar_size
                            .Worksheets("Database").Range(myCol & 61).Value = dpReb.longitudinal_rebar_yield_strength
                        ElseIf rebCount = 2 Then
                            .Worksheets("Database").Range(myCol & secStart + 6).Value = dpReb.longitudinal_rebar_quantity
                            .Worksheets("Database").Range(myCol & secStart + 7).Value = dpReb.longitudinal_rebar_size
                            .Worksheets("Database").Range(myCol & secStart + 8).Value = dpReb.longitudinal_rebar_cage_diameter
                        Else
                            .Worksheets("Database").Range(myCol & secStart + 10).Value = dpReb.longitudinal_rebar_quantity
                            .Worksheets("Database").Range(myCol & secStart + 11).Value = dpReb.longitudinal_rebar_size
                            .Worksheets("Database").Range(myCol & secStart + 12).Value = dpReb.longitudinal_rebar_cage_diameter
                        End If
                ...
                Dim soilStart As Integer = 180
                For Each dpSL As DrilledPierSoilLayer In dp.soil_layers
                    .Worksheets("Database").Range(myCol & soilCount).Value = dp.pier_id
                    .Worksheets("Database").Range(myCol & 179).Value = dpSL.soil_layer_id
                    .Worksheets("Database").Range(myCol & soilStart + 31 * 0 + (soilCount - 1)).Value = dpSL.bottom_depth
                    .Worksheets("Database").Range(myCol & soilStart + 31 * 1 + (soilCount - 1)).Value = dpSL.effective_soil_density
                    .Worksheets("Database").Range(myCol & soilStart + 31 * 2 + (soilCount - 1)).Value = dpSL.cohesion
                    .Worksheets("Database").Range(myCol & soilStart + 31 * 3 + (soilCount - 1)).Value = dpSL.friction_angle
                    .Worksheets("Database").Range(myCol & soilStart + 31 * 4 + (soilCount - 1)).Value = dpSL.skin_friction_override_comp
                    .Worksheets("Database").Range(myCol & soilStart + 31 * 5 + (soilCount - 1)).Value = dpSL.skin_friction_override_uplift
                    .Worksheets("Database").Range(myCol & soilStart + 31 * 6 + (soilCount - 1)).Value = dpSL.nominal_bearing_capacity
                    .Worksheets("Database").Range(myCol & soilStart + 31 * 7 + (soilCount - 1)).Value = dpSL.spt_blow_count
                    '.Worksheets("Soil Layers (SAPI)").Range("J" & soilRow).Value = dpSL.bearing_type_toggle
                ...
                If ds.Tables("Belled Details SQL").Rows.Count > 0 Then
                    '.Worksheets("Belled (SAPI)").Range(myCol & dpRow).Value = dp.pier_id
                    '.Worksheets("Belled (SAPI)").Range(myCol & dpRow).Value = dp.belled_details.belled_pier_id 'NEED TO FIND A WAY TO INCLUDE THIS
                    .Worksheets("Database").Range(myCol & 165).Value = dp.belled_details.belled_pier_option
                    .Worksheets("Database").Range(myCol & 166).Value = dp.belled_details.bottom_diameter_of_bell
                    .Worksheets("Database").Range(myCol & 167).Value = dp.belled_details.bell_input_type
                    .Worksheets("Database").Range(myCol & 168).Value = dp.belled_details.bell_angle
                    .Worksheets("Database").Range(myCol & 169).Value = dp.belled_details.bell_height
                    .Worksheets("Database").Range(myCol & 173).Value = dp.belled_details.bell_toe_height
                    .Worksheets("Database").Range(myCol & 175).Value = dp.belled_details.neglect_top_soil_layer
                    .Worksheets("Database").Range(myCol & 176).Value = dp.belled_details.swelling_expansive_soil
                    .Worksheets("Database").Range(myCol & 177).Value = dp.belled_details.depth_of_expansive_soil
                    .Worksheets("Database").Range(myCol & 178).Value = dp.belled_details.expansive_soil_force
                End If

                If ds.Tables("Embedded Details SQL").Rows.Count > 0 Then
                    '.Worksheets("Embedded (SAPI)").Range(myCol & dpRow).Value = dp.pier_id
                    '.Worksheets("Embedded (SAPI)").Range(myCol & dpRow).Value = dp.embed_details.embedded_id 'NEED TO FIND A WAY TO INCLUDE THIS
                    .Worksheets("Database").Range(myCol & 153).Value = dp.embed_details.embedded_pole_option
                    .Worksheets("Database").Range(myCol & 154).Value = dp.embed_details.encased_in_concrete
                    .Worksheets("Database").Range(myCol & 155).Value = dp.embed_details.pole_side_quantity
                    .Worksheets("Database").Range(myCol & 156).Value = dp.embed_details.pole_yield_strength
                    .Worksheets("Database").Range(myCol & 157).Value = dp.embed_details.pole_thickness
                    .Worksheets("Database").Range(myCol & 158).Value = dp.embed_details.embedded_pole_input_type
                    .Worksheets("Database").Range(myCol & 159).Value = dp.embed_details.pole_diameter_toc
                    .Worksheets("Database").Range(myCol & 160).Value = dp.embed_details.pole_top_diameter
                    .Worksheets("Database").Range(myCol & 161).Value = dp.embed_details.pole_bottom_diameter
                    .Worksheets("Database").Range(myCol & 162).Value = dp.embed_details.pole_section_length
                    .Worksheets("Database").Range(myCol & 163).Value = dp.embed_details.pole_taper_factor
                    .Worksheets("Database").Range(myCol & 164).Value = dp.embed_details.pole_bend_radius_override

                    'Dim depthEmbed As Integer = 65
                    'For Each eSec As DrilledPierEmbedSection In dp.embed_details.sections
                    '    '.Worksheets("Embedded (SAPI)").Range(myCol & dpRow).Value = dp.pier_id
                    '    'Section ID

                    '    If embedSecCount > 1 Then depth += 1
                    '    .Worksheets("Database").Range(myCol & depthEmbed).Value = eSec.pier_diameter

...
Partial Public Class DataTransfererCCIplate

#Region "Define"
    Private NewCCIplateWb As New Workbook
    Private prop_ExcelFilePath As String

    Public Property Connections As New List(Of CCIplate)
    Public Property sqlConnections As New List(Of CCIplate)
    'NEED TO UPDATE BELOW - reference template file
    Private Property CCIplateTemplatePath As String = "C:\Users\" & Environment.UserName & "\Documents\.NET Testing\Connection\Template\CCIplate (4.1.2.1) - Copy.xlsm"
    Private Property CCIplateFileType As DocumentFormat = DocumentFormat.Xlsm

    'Public Property connectionDS As New DataSet
    Public Property connectionDB As String
    Public Property connectionID As WindowsIdentity
...
#Region "Constructors"
    ...
    Public Sub New(ByVal MyDataSet As DataSet, ByVal LogOnUser As WindowsIdentity, ByVal ActiveDatabase As String, ByVal BU As String, ByVal Strucutre_ID As String)
        ds = MyDataSet
        connectionID = LogOnUser
        connectionDB = ActiveDatabase
...
#Region "Load Data"
    ...
    Sub CreateSQLConnections(ByRef connectionList As List(Of CCIplate))
        Dim refid As Integer
        Dim CCIplateLoader As String

        'Load data to get CCIplate details for the existing structure model
        For Each item As SQLParameter In CCIplateSQLDataTables()
            CCIplateLoader = QueryBuilderFromFile(queryPath & "CCIplate\" & item.sqlQuery).Replace("[EXISTING MODEL]", GetExistingModelQuery())
            DoDaSQL.sqlLoader(CCIplateLoader, item.sqlDatatable, ds, connectionDB, connectionID, "0")
        Next

        'Custom Section to transfer data for the CCIplate tool. Needs to be adjusted for each tool.
        'For Each CCIplateDataRow As DataRow In ds.Tables("CCIplate Plate Connections SQL").Rows 'this needs to be updated and reference connection_group
        '    refid = CType(CCIplateDataRow.Item("connection_id"), Integer)
        '    connectionList.Add(New CCIplate(CCIplateDataRow, refid))
        'Next
        For Each CCIplateDataRow As DataRow In ds.Tables("CCIplate Connection Group SQL").Rows 'this needs to be updated and reference connection_group
            refid = CType(CCIplateDataRow.Item("connection_group_id"), Integer)
            connectionList.Add(New CCIplate(CCIplateDataRow, refid))
        Next
    ...
    Public Function LoadFromEDS() As Boolean
        CreateSQLConnections(Connections)
    ...
    Public Sub LoadFromExcel()
        ...
        Connections.Add(New CCIplate(ExcelFilePath))

        'Dim refID As Integer
        'Dim refCol As String

        ''Custom Section to transfer data for the CCIplate tool. Needs to be adjusted for each tool.
        'For Each CCIplateDataRow As DataRow In ds.Tables("CCIplate General Details EXCEL").Rows
        '    refCol = "ID"
        '    refID = CType(CCIplateDataRow.Item(refCol), Integer)

        '    Connections.Add(New CCIplate(CCIplateDataRow, refID))
        'Next


        'Pull SQL data, if applicable, to compare with excel data
        CreateSQLConnections(sqlConnections)
...
#Region "Save Data"
    ...
    Sub Save1Connection(ByVal pc As CCIplate)

        Dim firstOne As Boolean = True
        Dim myPlateConnections As String = ""
        Dim myPlateDetails As String = ""
        Dim myPlateDetails2 As String = ""
        Dim myBasePlateOptions As String = ""

        Dim CCIplateSaver As String = QueryBuilderFromFile(queryPath & "CCIplate\CCIplate (IN_UP).sql")
        CCIplateSaver = CCIplateSaver.Replace("[BU NUMBER]", BUNumber)
        CCIplateSaver = CCIplateSaver.Replace("[STRUCTURE ID]", STR_ID)
        'CCIplateSaver = CCIplateSaver.Replace("[FOUNDATION TYPE]", "Pile")
        If pc.connection_group_id = 0 Or IsDBNull(pc.connection_group_id) Then
            CCIplateSaver = CCIplateSaver.Replace("'[Connection Group ID]'", "NULL")
        Else
            CCIplateSaver = CCIplateSaver.Replace("[Connection Group ID]", pc.connection_group_id.ToString)
        End If
        ...
        If isModelNeeded Then
            CCIplateSaver = CCIplateSaver.Replace("'[Model ID Needed]'", 1)
        Else
            CCIplateSaver = CCIplateSaver.Replace("'[Model ID Needed]'", 0)
        End If

        'Determine if new foundation group ID needs created. 
        If isconGroupNeeded Then
            CCIplateSaver = CCIplateSaver.Replace("'[Con GRP ID Needed]'", 1)
        Else
            CCIplateSaver = CCIplateSaver.Replace("'[Con GRP ID Needed]'", 0)
        End If

        For Each pcc As CCIplatePlateConnections In pc.plate_connections
            'line added below to avoid adding blank rows to tables when rows are removed. 
            'If Not IsNothing(pcsl.bottom_depth) Or Not IsNothing(pcsl.effective_soil_density) Or Not IsNothing(pcsl.cohesion) Or Not IsNothing(pcsl.friction_angle) Or Not IsNothing(pcsl.spt_blow_count) Or Not IsNothing(pcsl.ultimate_skin_friction_comp) Or Not IsNothing(pcsl.ultimate_skin_friction_uplift) Then
            Dim tempPlateConnection As String = InsertCCIplatePlateConnections(pcc)

            'If Not firstOne Then
            '    myPlateConnections = ",(" & tempPlateConnection & ")"
            'Else
            '    myPlateConnections = "(" & tempPlateConnection & ")"
            'End If
            ...
            For Each pcd As CCIplatePlateDetails In pc.plate_details
                Dim tempPlateDetail As String = InsertCCIplatePlateDetails(pcd)
                If pcc.local_id = pcd.local_id Then
                    If pcc.connection_type = "Base" Then
                        myPlateDetails = tempPlateDetail
                        For Each bpo As CCIplateBasePlateOptions In pc.base_plate_options
                            Dim tempBasePlateOption As String = InsertCCIplateBasePlateOptions(bpo)
                            myBasePlateOptions = tempBasePlateOption
                        Next
                        Exit For
                    ElseIf pcc.connection_type = "Flange" Then
                        If Not firstOne Then
                            Dim tempPlateDetail2 As String = InsertCCIplatePlateDetails(pcd)
                            myPlateDetails2 = tempPlateDetail2
                            Exit For
                        Else
                            myPlateDetails = tempPlateDetail
                        End If
            ...
            Dim subQuery As String = QueryBuilderFromFile(queryPath & "CCIplate\CCIplate (SUB QUERY).sql") ' Test {ath for new subQuery

            subQuery = subQuery.Replace("'[INSERT PLATE CONNECTIONS]'", myPlateConnections) '+ vbNewLine '.Replace("('[REINF DETAILS]')", IndividualDetailsFunction) + vbNewLine
            subQuery = subQuery.Replace("'[INSERT PLATE DETAILS]'", myPlateDetails)

            If pcc.connection_type = "Base" Then
                subQuery = subQuery.Replace("'[INSERT BASE PLATE OPTIONS]'", myBasePlateOptions)
            End If

            If pcc.connection_type = "Flange" Then
                'Deactivate queries
                subQuery = subQuery.Replace("INSERT INTO conn.base_plate_options VALUES ('[INSERT BASE PLATE OPTIONS]')", "--INSERT INTO conn.base_plate_options VALUES ('[INSERT BASE PLATE OPTIONS]')")
                'Activate additional queries
                subQuery = subQuery.Replace("--(Flange 2)INSERT", "INSERT")
                subQuery = subQuery.Replace("--(Flange 2)SELECT", "SELECT")
                subQuery = subQuery.Replace("'[INSERT PLATE DETAILS 2]'", myPlateDetails2)
            End If

            CCIplateSaver = CCIplateSaver.Replace("--SUBQUERY GOES HERE", subQuery)

            'Clear myplatedetails
            'myPlateDetails = ""

            'firstOne = False
            'End If
        Next 'Add Soil Layer INSERT statments
        'CCIplateSaver = CCIplateSaver.Replace("([INSERT ALL PLATE CONNECTIONS])", myPlateConnections)
        firstOne = True

        ''Determine if new Pile ID needs created
        'If isPileNeeded Then
        '    CCIplateSaver = CCIplateSaver.Replace("'[Pile ID Needed]'", 1)
        'Else
        '    CCIplateSaver = CCIplateSaver.Replace("'[Pile ID Needed]'", 0)
        'End If

        'CCIplateSaver = CCIplateSaver.Replace("[INSERT ALL PILE DETAILS]", InsertPileDetail(pc))
        'CCIplateSaver = CCIplateSaver.Replace("[CONFIGURATION]", pc.pile_group_config.ToString)

        ''If pc.pile_id = 0 Or IsDBNull(pc.pile_id) Then 'Don't need since only performing insert commands. 
        'If pc.pile_soil_capacity_given = False And pc.pile_shape <> "H-Pile" Then
        '    For Each pcsl As PileSoilLayer In pc.soil_layers
        '        'line added below to avoid adding blank rows to tables when rows are removed. 
        '        If Not IsNothing(pcsl.bottom_depth) Or Not IsNothing(pcsl.effective_soil_density) Or Not IsNothing(pcsl.cohesion) Or Not IsNothing(pcsl.friction_angle) Or Not IsNothing(pcsl.spt_blow_count) Or Not IsNothing(pcsl.ultimate_skin_friction_comp) Or Not IsNothing(pcsl.ultimate_skin_friction_uplift) Then
        '            Dim tempSoilLayer As String = InsertPileSoilLayer(pcsl)
        ...
        '        End If
        '    Next 'Add Soil Layer INSERT statments
        '    CCIplateSaver = CCIplateSaver.Replace("([INSERT ALL SOIL LAYERS])", mySoils)
        '    firstOne = True
        'Else
        '    CCIplateSaver = CCIplateSaver.Replace("INSERT INTO fnd.pile_soil_layer VALUES ([INSERT ALL SOIL LAYERS])", "--INSERT INTO fnd.pile_soil_layer VALUES ([INSERT ALL SOIL LAYERS])")
        'End If

        'If pc.pile_group_config = "Asymmetric" Then
        '    'CCIplateSaver = CCIplateSaver.Replace("[INSERT ALL PILE LOCATIONS]", InsertPileLocation(dp.embed_details))

        '    For Each pcpl As PileLocation In pc.pile_locations
        '        'line added below to avoid adding blank rows to tables when rows are removed. 
        '        If Not IsNothing(pcpl.pile_x_coordinate) Or Not IsNothing(pcpl.pile_y_coordinate) Then
        '            Dim tempLocation As String = InsertPileLocation(pcpl)
        ...
        '        firstOne = False
        '    Next
        '    CCIplateSaver = CCIplateSaver.Replace("([INSERT ALL PILE LOCATIONS])", myLocations)
        'Else
        '    CCIplateSaver = CCIplateSaver.Replace("BEGIN IF @IsCONFIG = 'Asymmetric'", "--BEGIN IF @IsCONFIG = 'Asymmetric'")
        '    CCIplateSaver = CCIplateSaver.Replace("INSERT INTO fnd.pile_location VALUES ([INSERT ALL PILE LOCATIONS]) End", "--INSERT INTO fnd.pile_location VALUES ([INSERT ALL PILE LOCATIONS]) End")
        ...
        '    CCIplateSaver = CCIplateSaver.Replace("BEGIN IF @IsCONFIG = 'Asymmetric'", "--BEGIN IF @IsCONFIG = 'Asymmetric'")
        '    CCIplateSaver = CCIplateSaver.Replace("INSERT INTO pile_soil_layer VALUES ([INSERT ALL SOIL LAYERS])", "--INSERT INTO pile_soil_layer VALUES ([INSERT ALL SOIL LAYERS])")
        '    CCIplateSaver = CCIplateSaver.Replace("INSERT INTO pile_location VALUES ([INSERT ALL PILE LOCATIONS]) End", "--INSERT INTO pile_location VALUES ([INSERT ALL PILE LOCATIONS]) End")

        '    Dim tempUpdater As String = ""
        '    tempUpdater += UpdatePileDetail(pc)

        '    If pc.pile_soil_capacity_given = False And pc.pile_shape <> "H-Pile" Then
        '        For Each pcsl As PileSoilLayer In pc.soil_layers
        '            If pcsl.soil_layer_id = 0 Or IsDBNull(pcsl.soil_layer_id) Then
        '                tempUpdater += "INSERT INTO pile_soil_layer VALUES (" & InsertPileSoilLayer(pcsl) & ") " & vbNewLine
        '            Else
        '                tempUpdater += UpdatePileSoilLayer(pcsl)
        '            End If
        '        Next
        '    End If

        '    'CCIplateSaver = CCIplateSaver.Replace("(SELECT * FROM TEMPORARY)", tempUpdater)
        ...
        '        For Each pcpl As PileLocation In pc.pile_locations
        '            If pcpl.location_id = 0 Or IsDBNull(pcpl.location_id) Then
        '                tempUpdater += "INSERT INTO pile_location VALUES (" & InsertPileLocation(pcpl) & ") " & vbNewLine
        '            Else
        '                tempUpdater += UpdatePileLocation(pcpl)
        '            End If
        '        Next
        ...
        '        '    'If pcpl.location_id = 0 Or IsDBNull(dp.embed_details.embedded_id) Then
        ...
        sqlSender(CCIplateSaver, connectionDB, connectionID, "0")
    End Sub

    Public Sub SaveToEDS()
        For Each pc As CCIplate In Connections
            Save1Connection(pc)
    ...
    '    For Each pf As CCIplate In Connections
    '        Dim soilRow As Integer = 57 'identify first row to copy data into Excel Sheet
    '        Dim locRow As Integer = 5
    ...
    Private Sub LoadNewConnection()
        NewCCIplateWb.LoadDocument(CCIplateTemplatePath, CCIplateFileType)
        NewCCIplateWb.BeginUpdate()
    End Sub

    Private Sub SaveAndCloseCCIplate()
        NewCCIplateWb.Calculate()
        NewCCIplateWb.EndUpdate()
        NewCCIplateWb.SaveDocument(ExcelFilePath, CCIplateFileType)
    End Sub
#End Region
...
#Region "SQL Insert Statements"
    ...
    Private Function InsertCCIplatePlateConnections(ByVal conpc As CCIplatePlateConnections) As String
        Dim insertString As String = ""

        'insertString += "@CCIplateID"
        'insertString += "," & IIf(IsNothing(conpc.connection_id), "Null", conpc.connection_id.ToString)
        insertString += "" & IIf(IsNothing(conpc.connection_elevation), "Null", conpc.connection_elevation.ToString)
        insertString += "," & IIf(IsNothing(conpc.connection_type), "Null", "'" & conpc.connection_type.ToString & "'")
        insertString += "," & IIf(IsNothing(conpc.bolt_configuration), "Null", "'" & conpc.bolt_configuration.ToString & "'")
        insertString += "," & IIf(IsNothing(conpc.local_id), "Null", conpc.local_id.ToString)

        Return insertString
    End Function

    Private Function InsertCCIplatePlateDetails(ByVal conpd As CCIplatePlateDetails) As String
        Dim insertString As String = ""

        insertString += "@CID"
        'insertString += "," & IIf(IsNothing(conpd.plate_id), "Null", conpd.plate_id.ToString)
        insertString += "," & IIf(IsNothing(conpd.plate_location), "Null", "'" & conpd.plate_location.ToString & "'")
        insertString += "," & IIf(IsNothing(conpd.plate_type), "Null", "'" & conpd.plate_type.ToString & "'")
        insertString += "," & IIf(IsNothing(conpd.plate_diameter), "Null", conpd.plate_diameter.ToString)
        insertString += "," & IIf(IsNothing(conpd.plate_thickness), "Null", conpd.plate_thickness.ToString)
        insertString += "," & IIf(IsNothing(conpd.plate_material), "Null", conpd.plate_material.ToString)
        insertString += "," & IIf(IsNothing(conpd.stiffener_configuration), "Null", conpd.stiffener_configuration.ToString)
        insertString += "," & IIf(IsNothing(conpd.stiffener_clear_space), "Null", conpd.stiffener_clear_space.ToString)
        insertString += "," & IIf(IsNothing(conpd.plate_check), "Null", "'" & conpd.plate_check.ToString & "'")
        insertString += "," & IIf(IsNothing(conpd.local_id), "Null", conpd.local_id.ToString)

        Return insertString
    End Function

#Region "SQL Insert Statements"
    Private Function InsertCCIplateBasePlateOptions(ByVal conbpo As CCIplateBasePlateOptions) As String
        Dim insertString As String = ""

        insertString += "@CID"
        'insertString += "," & IIf(IsNothing(conbpo.base_plate_option_id), "Null", conbpo.base_plate_option_id.ToString)
        insertString += "," & IIf(IsNothing(conbpo.anchor_rod_spacing), "Null", conbpo.anchor_rod_spacing.ToString)
        insertString += "," & IIf(IsNothing(conbpo.clip_distance), "Null", conbpo.clip_distance.ToString)
        insertString += "," & IIf(IsNothing(conbpo.barb_cl_elevation), "Null", conbpo.barb_cl_elevation.ToString)
        insertString += "," & IIf(IsNothing(conbpo.include_pole_reactions), "Null", "'" & conbpo.include_pole_reactions.ToString & "'")
        insertString += "," & IIf(IsNothing(conbpo.consider_ar_eccentricity), "Null", "'" & conbpo.consider_ar_eccentricity.ToString & "'")
        insertString += "," & IIf(IsNothing(conbpo.leg_mod_eccentricity), "Null", conbpo.leg_mod_eccentricity.ToString)
...
#Region "General"
    ...
    Private Function CCIplateSQLDataTables() As List(Of SQLParameter)
        ...
        MyParameters.Add(New SQLParameter("CCIplate Connection Group SQL", "CCIplate (SELECT Connection Group).sql"))
        MyParameters.Add(New SQLParameter("CCIplate Plate Connections SQL", "CCIplate (SELECT Plate Connections).sql"))
        MyParameters.Add(New SQLParameter("CCIplate Plate Details SQL", "CCIplate (SELECT Plate Details).sql"))
        MyParameters.Add(New SQLParameter("CCIplate Base Plate Options SQL", "CCIplate (SELECT Base Plate Options).sql"))

        Return MyParameters
    End Function

    Private Function CCIplateExcelDTParameters() As List(Of EXCELDTParameter)
        Dim MyParameters As New List(Of EXCELDTParameter)

        MyParameters.Add(New EXCELDTParameter("CCIplate Plate Connections EXCEL", "B2:F18", "SAPI"))
        MyParameters.Add(New EXCELDTParameter("CCIplate Plate Details EXCEL", "I2:R33", "SAPI"))
...
#End Region
    ...
    '    Function CheckChanges(ByVal xlPile As CCIplate, ByVal sqlPile As CCIplate) As Boolean
